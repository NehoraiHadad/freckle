...
async function EntityTableSection({
  slug,
  capability,
  searchParams,
}: {
  ...
  let data: Array<Record<string, unknown> & { id: string }> = []
...
export default async function GenericEntityPage({ params, searchParams }: EntityPageProps) {
  ...
  // Verify this capability exists on the product
  ...
  return (
    <div className="space-y-6">
      ...
      <Suspense fallback={<EntityTableSkeleton />}>
        <EntityTableSection slug={slug} capability={capability} searchParams={sp} />
      </Suspense>
    </div>
  )
}
